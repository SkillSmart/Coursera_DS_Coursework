---
title: "PracticalML_PeerAssignment_Writeup"
...
This document describes the analysis done for the prediction assignment of the practical machine learning course.
...
We can now train a classifier with the training data. To do that we will use parallelise the processing with the foreach and doParallel package : we call registerDoParallel to instantiate the configuration. (By default it assigns half of the cores available on your system, for me it's 4, because of hyperthreading) So we ask to process 4 random forest with 150 trees each and combine then to have a random forest model with a total of 600 trees.
```{r}
registerDoParallel()
model <- randomForest(classe ~ ., training, ntree=500, mtry=32)
```
